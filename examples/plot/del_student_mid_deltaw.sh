...
#             for adjust in 0
#             do
#                 # for METHOD in weight global_datasetgrad_divmag_iter
#                 for METHOD in weight global_datasetgrad_divmag_iter 
#                 do
#                     total_ratio=${ratio}
#                     init_ratio=${ratio}
...
#                     NAME=resnet18_${DATASET_ABBR}_lr5e-3_iter30000_feat0_wd1e-4_mmt0_1
#                     DIR=results/baseline/finetune
#                     DIR=${DIR}/${NAME}

#                     SAVE_DIR=results/plot/del_weight_student/adjust${adjust}/${NAME}

#                     CUDA_VISIBLE_DEVICES=$1 \
#                     python -u fineprune/plot/del_student_weight.py \
#                     --iterations ${iter} \
...
#                     --checkpoint $DIR/ckpt.pth \
#                     --adjust_iteration $adjust \
#                     # &
...
do
    # for ratio in 0 
    for ratio in 0 0.2 0.3 0.4 0.6 0.7 0.9
    do
        for trial_iter in  1000
        do
            total_ratio=${ratio}
            init_ratio=${ratio}
            per_ratio=0.1
...
total${total_ratio}_init${init_ratio}_per${per_ratio}_int${interval}_trainall_\
lr${lr}_iter${iter}_feat${lmda}_wd${wd}_mmt${mmt}_${id}
            DIR=results/plot/del_student_weight/middeltaw_inter0.1/$ratio
            CKPT_NAME=resnet18_${DATASET_ABBR}_lr5e-3_iter10000_feat0_wd1e-4_mmt0_1
            CKPT_DIR=results/baseline/finetune
