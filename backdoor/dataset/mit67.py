...
class MIT67Data(data.Dataset):
    def __init__(self, root, is_train=False, transform=None, shots=-1, seed=0, preload=False):
        ...
        for name in cls:
            if '.txt' not in name:
                self.cls_names.append(name.split('\\')[-1])

        # print(self.cls_names, len(self.cls_names))
        # input('ljw')

        if is_train:
            mapfile = os.path.join(root, 'Images', 'TrainImages.txt')
        else:
            mapfile = os.path.join(root, 'Images', 'TestImages.txt')
