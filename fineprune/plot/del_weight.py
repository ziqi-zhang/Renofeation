...
def eval_model(model, ref_model, train_loader, test_loader, args, adversary=None):
    ...
    for batch, label in test_loader:
        ...
        correct += int((raw_pred == del_pred).sum())
    ...
    if adversary is not None:
        ...
        return correct_ratio, raw_adv_correct_ratio, del_adv_correct_ratio
    else:
...
if __name__=="__main__":
    ...
    test_teacher.eval()
    test_student = load_student(args.checkpoint, args, train_loader.dataset.num_classes)
    test_student.eval()
    ...
    save_path = osp.join(args.output_dir, f"{args.method}.log")
    f = open(save_path, "w")
    student_correct = eval_model(test_student, ref_model, train_loader, test_loader, args)
    log = f"Student correct ratio {student_correct:.2f}"
    print(log)
    f.write(log+"\n")
    teacher_correct, raw_adv_correct, del_adv_correct = eval_model(test_teacher, ref_model, train_loader, test_loader, args, adversary)
    log = (
        f"Teacher correct ratio {teacher_correct:.2f}, "
