...
class SNIPPruner(Finetuner):
    def __init__(
        ...
        super(SNIPPruner, self).__init__(
        ...
        for batch, label in self.train_loader:
            ...
            for name, module in self.model.named_modules():
                if ( isinstance(module, nn.Conv2d) ):
                    # 1
                    # module.grad_log.add_(torch.abs(module.weight.grad.data))
                    # 2
                    # module.grad_log.add_(module.weight.grad.data)
                    # 3
                    taylor = torch.abs(module.weight.grad.data * module.weight.data)
                    module.grad_log.add_(taylor)
                    # 4
                    # taylor = module.weight.grad.data * module.weight.data
                    # module.grad_log.add_(taylor)
            # break
    ...
    def normalize_ranks(self):
        for name, module in self.model.named_modules():
            if ( isinstance(module, nn.Conv2d) ):
                taylor = torch.abs(module.grad_log)
                shape = taylor.shape
    ...
    def conduct_prune(self, prune_ratio):
        ...
        for name, module in model.named_modules():
            if ( isinstance(module, nn.Conv2d) ):
                ...
                conv_weights[index:(index+size)] = module.grad_log.view(-1).abs().clone()
                index += size
        ...
        for name, module in model.named_modules():
            if ( isinstance(module, nn.Conv2d) ):
                grad_copy = module.grad_log.clone().abs().cpu()
                # mask = grad_copy.gt(thre).float()
                mask = grad_copy.lt(thre).float()
    ...
    def snip_weight_prune(self, prune_ratio, iteration):
        ...
        for name, module in self.model.named_modules():
            if ( isinstance(module, nn.Conv2d) ):
                module.grad_log = torch.FloatTensor(module.weight.shape).zero_().cuda()
