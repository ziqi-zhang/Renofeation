...
class ForwardBackwardGrad(GlobalDatasetGradOptimIter):
    def __init__(
        self,
        args,
        model,
        ...
        super(ForwardBackwardGrad, self).__init__(
    ...
    def inv_train(self):
        print(f"Inverse train one epoch...")
        self.model.eval()
        self.model.zero_grad()
        ...
        for name, module in self.model.named_modules():
            if ( isinstance(module, nn.Conv2d) ):
                module.weight.backward_grad = torch.abs(module.weight.grad).cpu()
        self.model.zero_grad()
    ...
    def process_epoch(self):
        ...
        for module in self.model.modules():
            if ( isinstance(module, nn.Conv2d) ):
                module.pretrained_weight = module.weight.clone()
        ...
        for module in self.model.modules():
            if ( isinstance(module, nn.Conv2d) ):
                module.weight.forward_grad = torch.abs(module.weight - module.pretrained_weight).cpu()
                module.forward_weight = module.weight.clone()
        
        self.inv_train()
        for module in self.model.modules():
            if ( isinstance(module, nn.Conv2d) ):
                module.weight.grad_log = module.weight.forward_grad + module.weight.backward_grad
                del module.weight.forward_grad
                del module.weight.backward_grad
